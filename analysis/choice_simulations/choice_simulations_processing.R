...
obs_sum_stats_mixed <- c(get_sum_stats(as.matrix(brands_2008_mixed), components, all_zips, angles = FALSE),
...
obs_predicted_pca_mixed <- predict(pca, data.frame(t(c(obs_sum_stats_mixed))))
...
obs_predicted_umap_mixed <- predict(umap, t(as.matrix(obs_sum_stats_mixed)))
...
pca_plot <- ggplot(pca_plot_data, aes(x = PC1, y = PC2)) + geom_point(aes(color = group, shape = type, size = type)) + 
  scale_color_manual(values = group_colors, name = NULL, guide = guide_legend(override.aes = list(color = c("#0072B2", "#D55E00", "#009E73", "black"), shape = c(19, 19, 19, 17), size = c(1, 1, 1, 3)))) +
  scale_shape_manual(guide = "none", values = c(19, 17)) + scale_size_manual(guide = "none", values = c(1, 3)) +
...
plot_grid(pca_plot + theme(legend.position="none"), umap_plot + theme(legend.position="none"),
          legend, labels = c("A", "B", NULL), align = "hv", nrow = 1, rel_widths = c(2, 2, 0.6))
dev.off()

#create mixed pca plot
pca_plot_data_mixed <- as.data.frame(rbind(pca$x[, 1:2], t(as.matrix(obs_predicted_pca[1:2])), t(as.matrix(obs_predicted_pca_mixed[1:2]))))
pca_plot_data_mixed$group <- factor(c(rep("Null", nrow(sum_stats_a)), rep("Copy", nrow(sum_stats_b)), rep("Dist", nrow(sum_stats_c)), "Obs", "Obs (Mixed)"), levels = c("Null", "Copy", "Dist", "Obs", "Obs (Mixed)"))
pca_plot_data_mixed$type <- as.factor(c(rep(0, nrow(sum_stats_a)), rep(0, nrow(sum_stats_b)), rep(0, nrow(sum_stats_c)), 1, 2))
group_colors <- c(scales::alpha("#0072B2", opacity), scales::alpha("#D55E00", opacity), scales::alpha("#009E73", opacity), "black", "black")
pca_plot_mixed <- ggplot(pca_plot_data_mixed, aes(x = PC1, y = PC2)) + geom_point(aes(color = group, shape = type, size = type)) + 
  scale_color_manual(values = group_colors, name = NULL, guide = guide_legend(override.aes = list(color = c("#0072B2", "#D55E00", "#009E73", "black", "black"), shape = c(19, 19, 19, 17, 15), size = c(1, 1, 1, 3, 3)))) +
  scale_shape_manual(guide = "none", values = c(19, 17, 15)) + scale_size_manual(guide = "none", values = c(1, 3, 3)) +
  labs(x = "PC1", y = "PC2") + theme_linedraw()

#create mixed umap plot
umap_plot_data_mixed <- as.data.frame(rbind(umap$layout, obs_predicted_umap, obs_predicted_umap_mixed))
umap_plot_data_mixed$group <- factor(c(rep("Null", nrow(sum_stats_a)), rep("Copy", nrow(sum_stats_b)), rep("Dist", nrow(sum_stats_c)), "Obs", "Obs (Mixed)"), levels = c("Null", "Copy", "Dist", "Obs", "Obs (Mixed)"))
umap_plot_data_mixed$type <- as.factor(c(rep(0, nrow(sum_stats_a)), rep(0, nrow(sum_stats_b)), rep(0, nrow(sum_stats_c)), 1, 2))
group_colors <- c(scales::alpha("#0072B2", opacity), scales::alpha("#D55E00", opacity), scales::alpha("#009E73", opacity), "black", "black")
umap_plot_mixed <- ggplot(umap_plot_data_mixed, aes(x = V1, y = V2)) + geom_point(aes(color = group, shape = type, size = type)) + 
  scale_color_manual(values = group_colors, name = NULL, guide = guide_legend(override.aes = list(color = c("#0072B2", "#D55E00", "#009E73", "black", "black"), shape = c(19, 19, 19, 17, 15), size = c(1, 1, 1, 3, 3)))) +
  scale_shape_manual(guide = "none", values = c(19, 17, 15)) + scale_size_manual(guide = "none", values = c(1, 3, 3)) +
  labs(x = "UMAP1", y = "UMAP2") + theme_linedraw()

#save mixed plot
png("analysis/choice_simulations/choice_simulations_mixed.png", units = "in", width = 8, height = 3.5, res = 300)
legend <- get_legend(pca_plot_mixed)
plot_grid(pca_plot_mixed + theme(legend.position="none"), umap_plot_mixed + theme(legend.position="none"),
          legend, labels = c("A", "B", NULL), align = "hv", nrow = 1, rel_widths = c(2, 2, 0.6))
dev.off()
