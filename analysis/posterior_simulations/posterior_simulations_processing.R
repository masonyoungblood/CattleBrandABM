...
load("analysis/posterior_simulations/posterior_simulations.RData")
...
sum_stats_b <- do.call("rbind", lapply(1:length(posterior_simulations$sum_stats), function(x){c(t(as.matrix(posterior_simulations$sum_stats[[x]])))}))

#do pca
pca <- prcomp(sum_stats_a, scale = TRUE)
posterior_predicted_pca <- predict(pca, sum_stats_b)
...
umap <- umap(sum_stats_a, method = "umap-learn", config = config)
posterior_predicted_umap <- predict(umap, sum_stats_b)
...
#create pca plot
pca_plot_data <- as.data.frame(rbind(pca$x[, 1:2], posterior_predicted_pca[, 1:2], t(as.matrix(obs_predicted_pca[1:2]))))
pca_plot_data$group <- factor(c(rep("Prior", nrow(sum_stats_a)), rep("Posterior", nrow(sum_stats_b)), "Obs"), levels = c("Prior", "Posterior", "Obs"))
...
#create umap plot
umap_plot_data <- as.data.frame(rbind(umap$layout, posterior_predicted_umap, obs_predicted_umap))
umap_plot_data$group <- factor(c(rep("Prior", nrow(sum_stats_a)), rep("Posterior", nrow(sum_stats_b)), "Obs"), levels = c("Prior", "Posterior", "Obs"))
...
#save plot
png("analysis/posterior_simulations/posterior_simulations.png", units = "in", width = 8, height = 3.5, res = 300)
