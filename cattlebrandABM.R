...
brand_generator <-
  function(brands,
    ...
    copy_rows <-
      which(brands[, 9] %fin% as.numeric(names(which(
        zip_dists[, which(colnames(zip_dists) == zip)] <= copy_radius
      ))))
    dist_rows <-
      which(brands[, 9] %fin% as.numeric(names(which(
        zip_dists[, which(colnames(zip_dists) == zip)] <= dist_radius
      ))))
    
    #generate frequencies of components within the copy radius, where missing components are assigned 0
    copy_components <-
      (as.numeric(Rfast::Table(c(
        0:nrow(components), brands[copy_rows, 1:4]
      ))) - 1)[-1]
    
    #generate frequencies of components within the dist radius, where missing components are assigned 0
    dist_components <-
      (as.numeric(Rfast::Table(c(
        0:nrow(components), brands[dist_rows, 1:4]
      ))) - 1)[-1]
    ...
    sim_brand <-
      sample(
        1:nrow(components),
        sample(1:4, 1, prob = dpois(c(1, 2, 3, 4), complexity)),
        replace = TRUE,
        prob = ((copy_components + 1) ^ copy_strength) * ((1 / (
          dist_components + 1
        )) ^ dist_strength)
      )
    ...
    if (angles) {
      ...
      copy_angles <-
        sapply(sim_brand, function(x) {
          (as.numeric(Rfast::Table(c(0:9, brands[copy_rows, 5:8][which(brands[copy_rows, 1:4] == x)]))) -
             1)[-1]
        })
      ...
      dist_angles <-
        sapply(sim_brand, function(x) {
          (as.numeric(Rfast::Table(c(0:9, brands[dist_rows, 5:8][which(brands[dist_rows, 1:4] == x)]))) -
             1)[-1]
        })
      ...
      to_rot <-
        which(!is.na(components$rotatable)[sim_brand] &
                sample(
                  c(TRUE, FALSE),
                  length(sim_brand),
                  replace = TRUE,
                  prob = c(rot_prob, 1 - rot_prob)
                ))
      ...
      if (length(to_rot) > 0) {
        for (m in to_rot) {
          ...
          if (length(components$rotatable[[sim_brand[m]]]) == 1) {
            ...
            sim_angles[m] <-
              sample(components$rotatable[[sim_brand[m]]],
                     1,
                     prob = ((copy_angles[, m][components$rotatable[[sim_brand[m]]]] + 1) ^
                               copy_strength) * ((1 / (
                                 dist_angles[, m][components$rotatable[[sim_brand[m]]]] + 1
                               )) ^ dist_strength))
          }
        }
      }
    ...
    if (!angles) {
      ...
      return(c(c(sim_brand, rep(
        0, 4 - length(sim_brand)
      )), rep(0, 4)))
    } else{
...
cattlebrandABM <-
  function(init_brands,
    ...
    if (edit_dist_prop > 0) {
      output <- matrix(NA, nrow = length(sampling_years), ncol = 9)
    }
    if (edit_dist_prop == 0) {
      output <- matrix(NA, nrow = length(sampling_years), ncol = 8)
    }
    ...
    for (i in 1:t_steps) {
      ...
      #generate new brands
      new_brands <-
        t(sapply(1:n_new, function(x) {
          brand_generator(
            brands = running_brands,
            components = components,
            zip = new_zips[x],
            zip_dists = zip_dists,
            rot_prob = rot_prob,
            complexity = complexity,
            copy_radius = copy_radius,
      ...
      running_brands <-
        running_brands[-sample(1:nrow(running_brands), n_old),]
      ...
      if (i %fin% c(sampling_years - init_year)) {
        ...
        output[match(i, sampling_years - init_year),] <-
          get_sum_stats(
            running_brands = running_brands,
            components = components,
            all_zips = all_zips,
            angles = angles,
            edit_dist_prop = edit_dist_prop
          )
      }
...
get_sum_stats <-
  function(running_brands,
    ...
    if (!angles) {
      ...
      if (edit_dist_prop > 0) {
        ...
        edit_dist_mat <-
          adist(sapply(sample(
            nrow(running_brands),
            nrow(running_brands) * edit_dist_prop,
            replace = FALSE
          ), function(x) {
            intToUtf8(running_brands[x, 1:4])
          }))
      }
      
      #get component frequencies
      comp_freqs <-
        as.numeric(sort(Rfast::Table(c(
          running_brands[, 1:4]
        )[which(c(running_brands[, 1:4]) > 0)]), decreasing = TRUE))
      
      #construct components by zip codes matrix
      comp_beta_zip_mat <-
        t(sapply(1:length(all_zips$zip), function(x) {
          (as.numeric(Rfast::Table(c(
            0:nrow(components), running_brands[which(running_brands[, 9] == all_zips$zip[x]), 1:4]
          ))) - 1)[-1]
        }))
      if (length(which(Rfast::colsums(comp_beta_zip_mat) == 0)) > 0) {
        comp_beta_zip_mat <-
          comp_beta_zip_mat[,-which(Rfast::colsums(comp_beta_zip_mat) == 0)] #remove empty columns
      }
      if (length(which(Rfast::rowsums(comp_beta_zip_mat) == 0)) > 0) {
        comp_beta_zip_mat <-
          comp_beta_zip_mat[-which(Rfast::rowsums(comp_beta_zip_mat) == 0),] #remove empty rows
      }
      ...
      comp_beta_county_mat <-
        t(sapply(1:length(unique_counties), function(x) {
          (as.numeric(Rfast::Table(c(
            0:nrow(components), running_brands[which(running_brands[, 9] %fin% all_zips$zip[which(all_zips$county == unique_counties[x])]), 1:4]
          ))) - 1)[-1]
        }))
      if (length(which(Rfast::colsums(comp_beta_county_mat) == 0)) > 0) {
        comp_beta_county_mat <-
          comp_beta_county_mat[,-which(Rfast::colsums(comp_beta_county_mat) == 0)] #remove empty columns
      }
      if (length(which(Rfast::rowsums(comp_beta_county_mat) == 0)) > 0) {
        comp_beta_county_mat <-
          comp_beta_county_mat[-which(Rfast::rowsums(comp_beta_county_mat) == 0),] #remove empty rows
      }
    ...
    if (angles) {
      ...
      if (edit_dist_prop > 0) {
        ...
        edit_dist_mat <-
          adist(sapply(sample(
            ...
            intToUtf8(as.numeric(paste0(
              running_brands[x, 1:4], running_brands[x, 5:8]
            )))
          }))
      }
      ...
      comp_angle_combos <-
        c(sapply(1:nrow(running_brands), function(x) {
          paste(c(running_brands[x, 1:4]), c(running_brands[x, 5:8]))
        }))
      unique_comp_angle_combos <-
        unique(comp_angle_combos) #run this before the next line, because "0 0" needs to be accounted for later
      comp_angle_combos <-
        comp_angle_combos[-which(comp_angle_combos == "0 0")]
      
      #get component frequencies (where rotated components are treated as unique)
      comp_freqs <-
        as.numeric(sort(Rfast::Table(comp_angle_combos), decreasing = TRUE))
      
      #construct components by zip codes matrix
      comp_beta_zip_mat <-
        t(sapply(1:length(all_zips$zip), function(x) {
          (as.numeric(Rfast::Table(
            c(unique_comp_angle_combos, paste(
              c(running_brands[which(running_brands[, 9] == all_zips$zip[x]), 1:4]), c(running_brands[which(running_brands[, 9] == all_zips$zip[x]), 5:8])
            ))
          )) - 1)[-1]
        }))
      if (length(which(Rfast::colsums(comp_beta_zip_mat) == 0)) > 0) {
      ...
      comp_beta_county_mat <-
        t(sapply(1:length(unique_counties), function(x) {
          (as.numeric(Rfast::Table(
            c(unique_comp_angle_combos, paste(
              c(running_brands[which(running_brands[, 9] %fin% all_zips$zip[which(all_zips$county == unique_counties[x])]), 1:4]), c(running_brands[which(running_brands[, 9] %fin% all_zips$zip[which(all_zips$county == unique_counties[x])]), 5:8])
            ))
          )) - 1)[-1]
        }))
      if (length(which(Rfast::colsums(comp_beta_county_mat) == 0)) > 0) {
    ...
    if (edit_dist_prop > 0) {
      sum_stats <-
        c(
          max(comp_freqs) / sum(comp_freqs),
          #proportion of the most common component
          min(comp_freqs) / sum(comp_freqs),
          #proportion of the least common component
          hillR::hill_taxa(comp_freqs, q = 1),
          #shannon's diversity index
          hillR::hill_taxa(comp_freqs, q = 2),
          #simpson's diversity index
          hillR::hill_taxa_parti(comp_beta_zip_mat, q = 0)$region_similarity,
          #jaccard index (zip codes)
          hillR::hill_taxa_parti(comp_beta_zip_mat, q = 2)$local_similarity,
          #morisita-horn index (zip codes)
          hillR::hill_taxa_parti(comp_beta_county_mat, q = 0)$region_similarity,
          #jaccard index (counties)
          hillR::hill_taxa_parti(comp_beta_county_mat, q = 2)$local_similarity,
          #morisita-horn index (counties)
          mean(edit_dist_mat[upper.tri(edit_dist_mat)])
        ) #mean edit distance
    }
    if (edit_dist_prop == 0) {
      sum_stats <-
