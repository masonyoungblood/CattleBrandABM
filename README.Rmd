...
#get unique brands, ignoring zip codes
unique_brands <- brands[-which(duplicated(brands[, -9])), ]

#get quadrant for each brand in the dataset
brands_quadrants <- quad_thes$quadrant[match(unique_brands[, 9], quad_thes$zip)]

#get number of components per brand
n_components <- sapply(1:nrow(unique_brands), function(x){4 - length(which(unique_brands[x, 1:4] == 0))})
...
old_2 <- length(which(unique_brands[which(n_components == 2), 10] == 1990))
old_3 <- length(which(unique_brands[which(n_components == 3), 10] == 1990))
young_2 <- length(which(unique_brands[which(n_components == 2), 10] > 1990))
young_3 <- length(which(unique_brands[which(n_components == 3), 10] > 1990))

#what percent of component types are singletons
all_components <- c(unique_brands[, 1:4])
...
The total number of brands in the dataset, after removing duplicates within zip codes, is `r scales::comma(nrow(brands))`. Once we remove *all* duplicates we end up with `r scales::comma(nrow(unique_brands))` unique brands in the dataset. For unique brands with two components, `r scales::comma(old_2)` of them appear in 1990 and `r scales::comma(young_2)` of them only appear in 2008-2016. For unique brands with three components, `r scales::comma(old_3)` of them appear in 1990 and `r scales::comma(young_3)` of them only appear in 2008-2016. Among the unique brands, only `r percent_singleton`% of component types are singletons (i.e. only appear once). Here is the breakdown of the unique brands in terms of the four geographic quadrants described in the preregistration document, separately for two- and three-component brands:
